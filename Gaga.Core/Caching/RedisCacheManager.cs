...
namespace Gaga.Core.Caching
{
	...
	public partial class RedisCacheManager : ICacheManager
	{
		...
		//1//private readonly ICacheManager _perRequestCacheManager;
		#endregion

		#region Ctor

		public RedisCacheManager(GagaConfig config)
		{
			...
			//2//this._perRequestCacheManager = new PerRequestCacheManager(); //EngineContext.Current.Resolve<ICacheManager>();
		}
		...
		public virtual T Get<T>(string key)
		{
			...
			//3//if (_perRequestCacheManager.IsSet(key))
			//4//	return _perRequestCacheManager.Get<T>(key);
			...
			//5//_perRequestCacheManager.Set(key, result, 0);
			return result;
		}
		...
		public virtual bool IsSet(string key)
		{
			...
			//we use "PerRequestCacheManager" to cache a loaded object in memory for the current HTTP request.
			//this way we won't connect to Redis server 500 times per HTTP request (e.g. each time to load a locale or setting)
			//6//if (_perRequestCacheManager.IsSet(key))
			//7//	return true;
