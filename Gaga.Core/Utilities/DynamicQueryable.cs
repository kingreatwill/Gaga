...
NorthwindContext context = new NorthwindContext();
  ...
  //
  注意这个Library是对于IQueryable接口的扩展，而对于List<Member> _list来讲，使用Linq语句最初获取的是IEnumbale接口类型，所以要 在最开始的时候进行AsQueryable转换 。以前ADO.NET的时候，通过前台传过来的参数值，拼接SQL字符串语句，然后执行。现在也可以通过字符串拼接Linq语句然后执行。 
...
namespace Gaga.Core.Utilities//System.Linq.Dynamic
{
